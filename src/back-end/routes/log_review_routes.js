...
router.get('/', async (req, res) => {
  const allLogReviews = await LogReviewModel.find().populate('employee_id', '-_id username_id').populate({path: 'log_id', select: '-date -user_id', populate: {path: 'vehicle_id', select: 'asset_id -_id'}}).exec()
